...
Declare a prefix command for MAP named COMMAND-NAME."
  (let* ((root-map-sym (intern (format "%s-root-map" map)))
         ...
         (keys (when (plist-get args :keys)
                 (if (listp (plist-get args :keys))
                     (mapcar 'kbd (plist-get args :keys))
                   (list (kbd (plist-get args :keys))))))
         (evil-keys (when (plist-get args :evil-keys)
                      (if (listp (plist-get args :evil-keys))
                          (mapcar 'kbd (plist-get args :evil-keys))
                        (list (kbd (plist-get args :evil-keys))))))
